import { calculateCaloriesFromMacros, applyIngredientAdjustment, calculateIngredientMacros, calculateRecipeMacros, isWithinGoal, findLargestDeficitMacro, findBestIngredientForMacro } from './calculate';
import { Recipe, Macros, DailyPlan, MealType } from '@/lib/types';
...
export default function adjustRecipeIteratively(
    ...
    const isLastMeal = dailyPlan.meals.length === Object.keys(MealType).length - 1;
    ...
    for (let iteration = 0; iteration < maxIterations; iteration++) {
        ...
        const projectedDaily = calculateProjectedDaily(adjustedRecipe, dailyPlan);
        
        // Calculate total error for progress tracking
        const errors = {
            protein: Math.abs(currentMacros.protein - targetContribution.protein),
            carbs: Math.abs(currentMacros.carbs - targetContribution.carbs),
            fat: Math.abs(currentMacros.fat - targetContribution.fat)
        };
        
        totalError = (errors.protein + errors.carbs + errors.fat) / 
                          (targetContribution.protein + targetContribution.carbs + targetContribution.fat);
        ...
        if (errors.protein <= 3 && errors.carbs <= 3 && errors.fat <= 3) {
            if (isWithinCalorieBounds(projectedDaily, dailyPlan.goal)) {
                return { adjustments: Array.from(adjustments.values()), success: true };
            }
        }
        ...
        if (Math.abs(totalError - previousTotalError) < 0.01) {
            ...
            if (stagnationCount >= 3) {
                ...
                if (totalError < 0.3 || isWithinCalorieBounds(projectedDaily, dailyPlan.goal)) {
                    return { adjustments: Array.from(adjustments.values()), success: true };
                }
                break;
            }
        ...
        // Apply adjustment
        const ingredient = adjustedRecipe.ingredients.find(ri => ri.ingredient.id === bestAdjustment.ingredientId);
        if (ingredient) {
    ...
    if (!hasProgress || !withinBounds) {
        console.log('🔥 MACRO OPTIMIZATION FAILED - ACTIVATING CALORIE FALLBACK');
        
        const calorieDeficit = calculateCalorieDeficit(adjustedRecipe, dailyPlan);
...
function findBestMacroAdjustmentWithAdaptiveDamping(
    ...
    const scored = recipe.ingredients
        ...
        .map(ri => {
            const macroContent = ri.ingredient.macrosPer100g[criticalMacro.macro];
            const originalAmount = ri.originalAmount!;
            
            const canIncrease = criticalMacro.deficit > 0 && macroContent > 0 && ri.amount < originalAmount * 2.5;
            const canDecrease = criticalMacro.deficit < 0 && macroContent > 0 && ri.amount > originalAmount * 0.1;
            
            if (!canIncrease && !canDecrease) return null;

            // Efficiency score considering constraint headroom
            const constraintRoom = criticalMacro.deficit > 0 
    ...
    const newAmount = Math.max(minAmount, Math.min(maxAmount, best.amount + dampedChange));
...
function isWithinCalorieBounds(projectedDaily: any, goal: DailyGoal): boolean {
    const calorieMargin = goal.calories * goal.macros.plusMinusPercentage.calories / 100;
    return projectedDaily.calories >= (goal.calories - calorieMargin) 
...
function isConvergedWithCalorieBounds(
    ...
    if (useCalorieFirst) {
        ...
        const proteinDiff = Math.abs(currentMacros.protein - targetContribution.protein);
...
function findBestPrecisionCalorieAdjustment(
    ...
    const currentCalories = calculateCaloriesFromMacros(calculateRecipeMacros(recipe));
    const actualDeficit = targetCalories - currentCalories;
    
    console.log(`Precision targeting: ${actualDeficit.toFixed(1)} kcal (not ${overallDeficit.toFixed(1)} kcal)`);
    ...
    const scored = recipe.ingredients
        ...
        .map(ri => {
            ...
            const maxAmount = originalAmount * 2.5;
            const minAmount = originalAmount * 0.1;
            const adjustmentRoom = actualDeficit > 0 
            ...
            const canProvidePerfectAmount = maxContribution >= Math.abs(actualDeficit) * 0.3;
    ...
    const minAmount = originalAmount * 0.1;
    const maxAmount = originalAmount * 2.5;
    const newAmount = Math.max(minAmount, Math.min(maxAmount, best.amount + targetAmountChange));
...
function findBestMacroTargetAdjustment(recipe: Recipe, targetContribution: Macros, currentMacros: Macros): ParentAdjustment | null {
    const deficits = {
        protein: targetContribution.protein - currentMacros.protein,
        carbs: targetContribution.carbs - currentMacros.carbs,
        fat: targetContribution.fat - currentMacros.fat
    ...
    const criticalMacro = (['protein', 'carbs', 'fat'] as const)
        .map(macro => ({ macro, deficit: deficits[macro], abs: Math.abs(deficits[macro]) }))
        .filter(item => item.abs > 2.0)
        .sort((a, b) => b.abs - a.abs)[0];
    
    if (!criticalMacro) return null;
    
    console.log(`Targeting: ${criticalMacro.macro} (deficit: ${criticalMacro.deficit.toFixed(1)}g)`);
...
function calculateCalorieDeficit(recipe: Recipe, dailyPlan: DailyPlan): number {
    const currentRecipeCalories = calculateCaloriesFromMacros(calculateRecipeMacros(recipe));
    const existingCalories = dailyPlan.meals.reduce((sum, meal) => 
        sum + calculateCaloriesFromMacros(calculateRecipeMacros(meal.recipe)), 0
    );
...
function calculateProjectedDaily(recipe: Recipe, dailyPlan: DailyPlan) {
    ...
    const existingMacros = dailyPlan.meals.reduce((sum, meal) => {
...
function analyzeCurrentState(recipe: Recipe, dailyPlan: DailyPlan, targetContribution: Macros) {
    ...
    const totalTargetError = (proteinDiff + carbsDiff + fatDiff) / (targetContribution.protein + targetContribution.carbs + targetContribution.fat);
...
function executeCalorieFirstFallback(
    ...
    if (!isLastMeal) {
        ...
        return { 
            adjustments: [], 
            success: false,
            error: "Calorie-first fallback only available for last meal"
        };
    }
    ...
    for (let iteration = 0; iteration < MAX_FALLBACK_ITERATIONS; iteration++) {
        const projectedDaily = calculateProjectedDaily(adjustedRecipe, dailyPlan);
        console.log(adjustedRecipe, calculateRecipeMacros(adjustedRecipe), calculateCaloriesFromMacros(calculateRecipeMacros(adjustedRecipe)), projectedDaily)
        console.log(`Iteration ${iteration + 1}: Daily total ${projectedDaily.calories.toFixed(1)} kcal`);
        ...
        const actualDailyDeficit = dailyPlan.goal.calories - projectedDaily.calories;
        ...
        if (ingredient) {
            console.log(`🔥 CALORIE-FIRST: ${bestAdjustment.ingredientId} ${ingredient.amount}g → ${bestAdjustment.newAmount}g`);
            ingredient.amount = bestAdjustment.newAmount;
            adjustments.set(bestAdjustment.ingredientId, bestAdjustment);
        }
    }
    ...
    const finalProjected = calculateProjectedDaily(adjustedRecipe, dailyPlan);
    const withinCalorieBounds = isWithinCalorieBounds(finalProjected, dailyPlan.goal);
    
    console.log(`Final calorie bounds check: ${withinCalorieBounds}`);
    console.log(`Final daily total: ${finalProjected.calories.toFixed(1)} kcal`);
...
function findMostCalorieEfficientAdjustment(
    ...
    const rankedIngredients = recipe.ingredients
        ...
        .map(ri => {
            ...
            return {
                ingredient: ri,
                calorieDensity,
                adjustmentCapacity: calorieDeficit > 0 ? maxIncrease : maxDecrease,
                score: calorieDensity * (calorieDeficit > 0 ? maxIncrease : maxDecrease)
            };
        })
        .filter(item => item !== null)
        .sort((a, b) => b.score - a.score);
    ...
    const actualChange = Math.min(targetAmountChange, best.adjustmentCapacity) * direction;
    
    const minAmount = originalAmount * 0.1;
    const maxAmount = originalAmount * 2.5;
    const newAmount = Math.max(minAmount, Math.min(maxAmount, ingredient.amount + actualChange));
